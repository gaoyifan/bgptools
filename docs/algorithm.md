...
- Inputs: MRT file paths (`--mrt-file`), target ASNs (positional), `--ignore-private-asn`, `--origin-only`, and `--cache`.
...
When `--cache` is enabled, the computed ASN→range maps are serialized to a bincode file keyed by input file list, `ignore_private_asn`, and `origin_only`. Subsequent runs reuse the cache when the key matches.
